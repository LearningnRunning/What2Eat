...
from config.constants import (DEFAULT_ADDRESS_INFO_LIST, GRADE_COLORS,
                              GRADE_MAP, GUIDE_IMG_PATH, LOGO_IMG_PATH,
                              LOGO_SMALL_IMG_PATH, LOGO_TITLE_IMG_PATH,
                              PRIORITY_ORDER)
from streamlit_geolocation import streamlit_geolocation
from utils.data_loading import load_static_data
from utils.data_processing import (category_filters, get_filtered_data,
                                   grade_to_stars, hex_to_rgba,
                                   pick_random_diners, search_menu,
                                   select_radius)
from utils.geolocation import geocode, search_your_address
from utils.ui_components import (choice_avatar, display_results,
                                 generate_introduction, my_chat_message)
...
class SearchManager:
    ...
    def search_by_menu(self, menu_search, filtered_df):
        return filtered_df[
            filtered_df.apply(
                lambda row: search_menu(row, menu_search), 
                axis=1
            )
        ]
    ...
    def get_random_recommendations(self, filtered_df, num_to_select=5):
        ...
        return st.session_state.result_queue.pop(0) if st.session_state.result_queue else None
...
class What2EatApp:
    ...
    def ranking_page(self):
        ...
        # 지역 선택
        self.df_diner[['city', 'region']] = self.df_diner['diner_num_address'].str.split(' ', n=2, expand=True).iloc[:, :2]
        ...
        if city_label:
            ...
            center_latitude, center_longitude = ranked_df.iloc[0,4], ranked_df.iloc[0,5]
            # 각 음식점의 핀 정보 생성
    ...
    def chat_page(self):
        ...
        if len(df_geo_filtered):
            ...
            if search_option == "메뉴로 찾기":
                ...
                diner_category_lst = list(df_geo_filtered_real_review["diner_category_large"].unique())
