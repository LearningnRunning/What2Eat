...
df_diner = pd.read_excel('./diner_only.xlsx', sheet_name='diner', index_col = 0)
df_review = pd.read_excel('./diner_only.xlsx', sheet_name='review', index_col = 0)
...
# 기능창
    ...
    if bool(diner_category) and bool(region):
        ...
        # result_lst = Counter(result_df["name"].to_list()).most_common()
            ## 최적화
        result_lst = result_df_inner_join.value_counts().index
        ...
        for result in result_lst:
            try:
                ...
                # detail = result_df[result_df["name"] == result[0]].iloc[-1, :]
                    ## 쿼리문 대체
                detail_frame = result_df_inner_join.query(f'diner_name == {result}]')
                detail = detail_frame.iloc[-1]
                # X_Point
                # 키워드 문자열 정리
                '''
                ...
                '''
                    ## 정리
                if detail["diner_review_tags"] is not None:
                    detail_set = detail.drop_duplicates(subset = 'diner_name', keep='last')
                    diner_tags = " ".join(detail_set["diner_review_tags"].split('@'))
                color = 'darkblue'
                unlike = ''
                if len(bad_reviews) >= 5:
                    color = 'gray'
                ...
                if result[1] >= people_counts:
                    ...
                    if len(menu) >= 120:
                        menu = menu[:120] 
                    html = popup_html(detail,result[1], diner_tags, menu, unlike)
