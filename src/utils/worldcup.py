...
class WorldCupManager:
    ...
    def get_random_diners(self, n: int = 2) -> list[dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.api_url}/kakao/diners/filtered", params={"n": n}, timeout=20
            )
    ...
    def get_diner_by_idx(self, diner_idx: int) -> Optional[dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.api_url}/kakao/diners/{diner_idx}", timeout=5
            )
    ...
    def build_tournament_candidates(
        self, selected_diner: dict[str, Any], size: int = 8
    ) -> list[dict[str, Any]]:
        ...
        similar_ids = (
            self.get_similar_restaurants(selected_diner_idx)
            if selected_diner_idx
            else []
        )
        ...
        if len(similar_restaurants) < needed:
            ...
            existing_ids = {
                r["diner_idx"] for r in all_candidates + similar_restaurants
            }
    ...
    def show_initial_selection(self):
        ...
        st.markdown(
            "<h3 style='text-align:center;'>🎯 시작할 식당을 선택하세요</h3>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='text-align:center; color:gray;'>선택한 식당과 비슷한 맛집들로 토너먼트가 구성됩니다</p>",
            unsafe_allow_html=True,
        )
    ...
    def start_tournament_with_selection(self, selected_idx: int):
        ...
        if not candidates or len(candidates) < 8:
            st.error("토너먼트를 시작하기에 충분한 식당(8개)을 불러오지 못했습니다.")
            return
    ...
    def select_winner(self, winner_idx: int):
        """승자 선택 및 다음 라운드 진행"""
        winner = st.session_state.matches[st.session_state.current_match_index][
            winner_idx
        ]
    ...
    def render_statistics(self):
        ...
        with col3:
            if st.session_state.winners:
                ...
                winner_count = sum(
                    1 for d in selected_diners if d["diner_idx"] == winner["diner_idx"]
                )
        ...
        categories = [
            d.get("diner_category_large", "기타")
            for d in selected_diners
            if d.get("diner_category_large")
        ]
        ...
        with st.expander("📝 선택한 모든 식당 보기"):
            ...
            sorted_diners = sorted(
                diner_counts.values(), key=lambda x: x["count"], reverse=True
            )
            ...
            for item in sorted_diners:
                ...
                is_winner = (
                    st.session_state.winners
                    and diner["diner_idx"] == st.session_state.winners[0]["diner_idx"]
                )
    ...
    def render_worldcup_page(self):
        ...
        # 토너먼트 시작 전 - 초기 선택 화면
        if (
            not st.session_state.tournament_started
            or st.session_state.tournament_finished
        ):
        ...
        # 토너먼트 진행 중 - 현재 매치 표시
        if st.session_state.matches and st.session_state.current_match_index < len(
            st.session_state.matches
        ):
            st.markdown(
            ...
            current_match = st.session_state.matches[
